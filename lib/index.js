...
// publishing pid) into `dsh-desktop.conf`, and the host navigates to it, which
...
function sameFileContents(a, b) {
  try {
    ...
    return false; // a missing/unreadable destination must be written
  }
}
 ...
 * at `$DSH_HOME/dsh-desktop/dsh-desktop.conf`, rewriting just those two lines.
...
function publishAuthUrl(authUrl, pid) {
  ...
  published.pid = pid;
  const configPath = join(harnessHome(), "dsh-desktop", "dsh-desktop.conf");
  let current;
  try {
...
function createShortcut(config) {
  ...
  //
  // A host that is running right now keeps `dsh-desktop.exe` and the WebView2
  ...
  ctx.inject(["connection", "webServer"], (web) => {
    ...
    const publish = () => {
      try {
        ...
        console.log("dsh-desktop: published this process's authenticated Web URL for the desktop host");
        return true;
      } catch (error) {
        console.warn("dsh-desktop: could not publish the authenticated Web URL: " + errorMessage(error));
        return false;
      }
    };
...
export function apply(ctx, config) {
  ...
  try {
    ...
    console.warn("dsh-desktop: " + errorMessage(error));
  }
  try {
