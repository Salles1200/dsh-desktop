...
/** Shortcut name and window title used when the config does not override them. */
...
/**
 * Whether any profile still selects this bundle. `dsh plugin remove` and the Web
 * plugin manager both drop the name from `dsh.profile.bundles` before the
 * runtime is unloaded, so a dispose that no longer sees the name is an
 * uninstall, while an ordinary DSH shutdown or a configuration reload still
 * sees it and must leave the shortcut, host, and state directory alone.
 *
 * Reads that fail report "still selected": never delete on a read error.
 */
function bundleStillSelected() {
  const profilesDir = join(harnessHome(), "profiles");
  let entries;
  try {
    entries = readdirSync(profilesDir, { withFileTypes: true });
  } catch {
    return true; // no profiles to inspect: cannot prove an uninstall
  }
  let sawManifest = false;
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    try {
      const manifest = JSON.parse(readFileSync(join(profilesDir, entry.name, "package.json"), "utf8"));
      const bundles = manifest?.dsh?.profile?.bundles;
      if (!Array.isArray(bundles)) continue;
      sawManifest = true;
      if (bundles.includes(name)) return true;
    } catch {
      // a profile directory without a readable manifest: ignore it
    }
  }
  return !sawManifest;
}
...
/**
 * Delete the ICO files this plugin wrote earlier, so the state directory does not
 * accumulate one file per icon change. Best effort: a running host or the shell
 * may still hold one, and the next boot retries.
 */
function removeStaleIcons(dir, keep) {
  let entries;
  try {
    entries = readdirSync(dir);
  } catch {
    return; // unreadable directory: nothing to clean
  }
  for (const entry of entries) {
    if (entry === keep || !/^dsh(-[0-9a-f]{12})?\.ico$/.test(entry)) continue;
    try {
...
/**
 * Build the PowerShell snippet that makes the `.lnk` match this configuration.
 * The script decides for itself, because the fingerprint file cannot see the
 * real shortcut: a deleted or repointed `.lnk` leaves the fingerprint unchanged.
 * The icon path carries the icon's digest, so changed icon bytes already appear
 * here as a different IconLocation.
 */
function buildShortcutScript({ exePath, icoPath, shortcutName, workingDir, desktopDir }) {
  const label = psSingleQuote('"' + shortcutName + '"');
  const lines = [];
  lines.push("$ErrorActionPreference = 'Stop'");
  // Progress records serialize as CLIXML on a redirected stderr: keep logs clean.
  ...
  lines.push("$ok = $false");
  lines.push("if (Test-Path -LiteralPath $lnk) {");
  lines.push("  $now = $ws.CreateShortcut($lnk)");
  lines.push("  $ok = ($now.TargetPath -eq " + psSingleQuote(exePath) + ") -and ($now.WorkingDirectory -eq " + psSingleQuote(workingDir) + ") -and ($now.Description -eq " + psSingleQuote(shortcutName) + ")");
  if (icoPath) lines.push("  $ok = $ok -and ($now.IconLocation -eq " + psSingleQuote(icoPath + ",0") + ")");
  lines.push("}");
  lines.push("if ($ok) { [Console]::Out.WriteLine('dsh-simple-desktop: desktop shortcut ' + " + label + " + ' is already up to date'); exit 0 }");
  ...
  lines.push("$sc.Save()");
  lines.push("[Console]::Out.WriteLine('dsh-simple-desktop: created the ' + " + label + " + ' desktop shortcut')");
  return lines.join("\n");
}

/**
 * Build the PowerShell snippet that deletes the `.lnk` this plugin created — and
 * only that one: a shortcut whose target is not our host exe is left untouched,
 * so uninstalling never removes something the user made themselves.
 */
function buildShortcutRemovalScript({ exePath, shortcutName, desktopDir }) {
  const lines = [];
  lines.push("$ErrorActionPreference = 'Stop'");
  lines.push("$ProgressPreference = 'SilentlyContinue'");
  lines.push("$desktop = [Environment]::GetFolderPath('Desktop')");
  if (desktopDir) lines.push("$desktop = " + psSingleQuote(desktopDir));
  lines.push("$lnk = Join-Path $desktop (" + psSingleQuote(shortcutName + ".lnk") + ")");
  lines.push("if (-not (Test-Path -LiteralPath $lnk)) { [Console]::Out.WriteLine('dsh-simple-desktop: no desktop shortcut to remove'); exit 0 }");
  lines.push("$ws = New-Object -ComObject WScript.Shell");
  lines.push("$sc = $ws.CreateShortcut($lnk)");
  lines.push("if ($sc.TargetPath -ne " + psSingleQuote(exePath) + ") { [Console]::Out.WriteLine('dsh-simple-desktop: kept ' + $lnk + ' because it points at ' + $sc.TargetPath); exit 0 }");
  lines.push("Remove-Item -LiteralPath $lnk -Force");
  lines.push("[Console]::Out.WriteLine('dsh-simple-desktop: removed ' + $lnk)");
  return lines.join("\n");
}
...
function createShortcut(config) {
  ...
  const shortcutName = config.shortcutName ?? DEFAULT_SHORTCUT_NAME;
  ...
  //
  // The ICO file name carries a digest of its own bytes, because Windows caches
  // a shortcut's icon by file path: reusing one `dsh.ico` leaves Explorer on the
  // previous bitmap until its cache is rebuilt. Unchanged bytes are left alone,
  // earlier ICO files are cleaned up, and an unreadable PNG only warns — it must
  // not abort the host/config/shortcut work below.
  let icoPath = null;
  let icoDigest = "";
  if (iconPath) {
    try {
      ...
      icoPath = join(dir, "dsh-" + icoDigest.slice(0, 12) + ".ico");
      if (!fileHasBytes(icoPath, ico)) writeFileSync(icoPath, ico);
      removeStaleIcons(dir, icoPath);
    } catch (error) {
      console.warn("dsh-simple-desktop: could not use the shortcut icon (" + errorMessage(error) + "); continuing without one");
    }
  } else {
  ...
  // The shortcut on disk is the source of truth, so the script runs on every
  // boot and rewrites the `.lnk` only when it is missing or different. The
  // manifest stays as a record of the last successful run.
...
/**
 * Undo what this plugin installed: the desktop shortcut (only when it still
 * points at our host exe) and the state directory under `$DSH_HOME`. Both steps
 * are best effort — a desktop window that is still open keeps the host exe and
 * its WebView2 files locked, so a leftover path is reported instead of thrown.
 */
function cleanupInstallation(config) {
  const dir = join(harnessHome(), name);
  try {
    runPowerShell(buildShortcutRemovalScript({
      exePath: join(dir, name + ".exe"),
      shortcutName: config.shortcutName ?? DEFAULT_SHORTCUT_NAME,
      desktopDir: config.desktopDir,
    }));
  } catch (error) {
    console.warn("dsh-simple-desktop: could not remove the desktop shortcut: " + errorMessage(error));
  }
  try {
    rmSync(dir, { recursive: true, force: true });
    console.log("dsh-simple-desktop: removed the host state directory " + dir);
  } catch (error) {
    console.warn("dsh-simple-desktop: could not remove " + dir + " (" + errorMessage(error) + "); close the desktop window from its tray icon and delete that folder");
  }
}

/**
 * Clean up on uninstall rather than on shutdown. The disposer runs for every
 * dispose (DSH exiting, a configuration reload, an uninstall), so it first asks
 * whether any profile still selects this bundle and only removes the
 * installation when the answer is no.
 */
function registerUninstallCleanup(ctx, config) {
  if (typeof ctx.effect !== "function") return;
  ctx.effect(() => () => {
    try {
      if (bundleStillSelected()) return;
      cleanupInstallation(config);
    } catch (error) {
      console.warn("dsh-simple-desktop: uninstall cleanup failed: " + errorMessage(error));
    }
  });
}
...
export function apply(ctx, config) {
  ...
  try {
    registerUninstallCleanup(ctx, config ?? {});
  } catch (error) {
    console.warn("dsh-simple-desktop: " + errorMessage(error));
  }
}
