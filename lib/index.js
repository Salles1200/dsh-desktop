 ...
 * config, a `deepseek-color.png` beside the workspace root, then the bundled
 * `assets/default-icon.png`. Returns null when no icon is available (the
 * shortcut and window then fall back to the host's default icon).
 */
function resolveIconPath(config) {
  const candidates = [
    config.iconPath,
    join(process.cwd(), "deepseek-color.png"),
...
function createShortcut(config) {
  ...
  // Icon (optional): explicit iconPath > <workspace>/deepseek-color.png >
  // bundled default-icon.png. Without one, the shortcut and window fall back
  // to the host's default icon.
  if (iconPath) {
    writeFileSync(icoPath, pngToIco(readFileSync(iconPath)));
  } else {
    console.warn("dsh-desktop: no icon found; using the host's default icon (set iconPath, or place deepseek-color.png in the workspace)");
  }
